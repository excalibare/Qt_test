	...
	void scanlineFill(QPainter& painter, const Polygon& polygon) {
		...
		for (int y = ymin; y <= ymax; ++y) {
			...
			for (size_t i = 0; i < intersections.size(); i += 2) {
				...
				// TODO: 计划用自带的直线绘画函数代替drawline函数
				// lyc：所以我们的drawline有没有写这个↑参数形式的方法啊
				// drawBresenhamLine(painter, QPoint(intersections[i], y),QPoint(intersections[i + 1], y));
				// lines.append(Line(QPoint(intersections[i], y), QPoint(intersections[i + 1], y), lineWidth, currentLineColor, algorithm));
			}
		}
	}
